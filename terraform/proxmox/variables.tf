...
variable "omni_iso_images" {
  description = "Map of Talos versions to Omni ISO names (generated by update-tfvars.sh)"
  type        = map(string)
  default     = {}
}
...
variable "vm_configs" {
  description = "List of VM configurations. Each config specifies count, cpu, memory, disk, role, talos_version, and cluster."
  type = list(object({
    count         = number
    cpu           = number
    memory        = number
    ...
    talos_version = string # Talos version (e.g., "1.9.0")
    cluster       = string # Cluster name this VM belongs to
  }))
