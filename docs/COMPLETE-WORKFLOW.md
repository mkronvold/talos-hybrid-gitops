...
- Creates `terraform/proxmox/terraform.tfvars.dk1d` for the entire site
...
```bash
# Automatically detects all Talos versions used by clusters in site
...
**What this does:**
- Scans all cluster YAML files to find required Talos versions
- Downloads ISO from Omni with pre-baked credentials for each version
- Includes site labels: `site=dk1d`, `platform=proxmox`
- Includes platform-specific guest agent (qemu-guest-agent for Proxmox)
- Uploads ISO(s) to Proxmox via SCP
- ISOs stored with version in filename (e.g., `talos-omni-dk1d-1.9.0.iso`)

**Note:** Run once per site. ISOs for all Talos versions will be created automatically.

### Phase 4: Variables Update

Calculate Terraform variables from all cluster definitions in the site.
...
**What this does:**
- Scans all `cluster-*.yaml` files in site directory
- Extracts Talos versions and finds corresponding ISO URLs
- Groups VMs by role (controlplane/worker), size class, and Talos version
- Generates `vm_configs` in single site tfvars file: `terraform/proxmox/terraform.tfvars.dk1d`
- Creates backup of previous tfvars before updating

**Key principles:**
- One tfvars file per site (not per cluster)
- Aggregates VM requirements across all clusters
- Maps each VM config to the correct ISO for its cluster's Talos version
- Supports multiple clusters with different Talos versions in same site

**Example vm_configs generated:**
```hcl
# Site: dk1d with 3 clusters using 2 different Talos versions

# ISOs for different versions
omni_iso_urls = {
  "1.9.0" = "https://proxmox1:8006/api2/json/.../talos-omni-dk1d-1.9.0.iso"
  "1.8.3" = "https://proxmox1:8006/api2/json/.../talos-omni-dk1d-1.8.3.iso"
}
...
Deploy VMs using Terraform for the entire site.
...
- Uses single tfvars file: `terraform.tfvars.dk1d`
- Runs `terraform init` (if needed)
- Runs `terraform plan` to show changes (all VMs for all clusters)
- Runs `terraform apply` to create VMs
- VMs boot from appropriate versioned Omni ISO (based on talos_version)
- VMs automatically register with Omni (2-5 minutes)

**Important:** This provisions ALL VMs for ALL clusters in the site at once.

**Terraform output example:**
```
vm_names = [
  "dk1d-vm-001",  # Will be allocated to baseline cluster
  "dk1d-vm-002",
  "dk1d-vm-003",
  "dk1d-vm-004",
  "dk1d-vm-005",
  "dk1d-vm-006",  # Will be allocated to web cluster
  "dk1d-vm-007",
  ...
]
```
...
# 2. Configure Proxmox credentials (one file for entire site)
vim terraform/proxmox/terraform.tfvars.dk1d
...
# 4. Prepare ISOs (automatically detects all Talos versions needed)
./scripts/prepare-omni-iso.sh dk1d

# 5. Update Terraform variables (aggregates all clusters into one tfvars)
./scripts/update-tfvars.sh dk1d

# 6. Provision nodes (all VMs for all clusters)
./scripts/provision-nodes.sh dk1d
...
# Add labels: site=dk1d, platform=proxmox, size_class=4x8 or 8x16

# 9. Apply all clusters
./scripts/apply-clusters.sh dk1d

# 10. Wait for clusters to become ready (10-15 min)
watch -n 5 omnictl get clusters

# 11. Download kubeconfigs for all clusters
./scripts/get-kubeconfigs.sh dk1d --merge
...
# Prepare ISO if using new Talos version
./scripts/prepare-omni-iso.sh dk1d

# Update variables and provision (adds VMs for new cluster)
./scripts/update-tfvars.sh dk1d
./scripts/provision-nodes.sh dk1d
...
Note: The site tfvars file will be updated to include VMs for the new cluster.
...
# Remove cluster YAML file
rm clusters/omni/dk1d/cluster-baseline.yaml

# Update tfvars to remove VMs for deleted cluster
./scripts/update-tfvars.sh dk1d

# Apply Terraform to destroy unused VMs
cd terraform/proxmox
terraform apply -var-file=terraform.tfvars.dk1d

# Or destroy all VMs for entire site
terraform destroy -var-file=terraform.tfvars.dk1d
```

Note: The per-site tfvars strategy means destroying individual clusters requires updating tfvars and re-applying, not destroying all infrastructure.
